...
new Vue({
	...
	computed: {
		...
		tableStyle: function() {
			...
			var disY = 210;
			var arr = [ 
				{x : 0, y : 0}, {x : 17, y : 0}, {x : 0, y : 1}, {x : 1, y : 1}, {x : 12, y : 1},
			    {x : 13, y : 1}, {x : 14, y : 1}, {x : 15, y : 1}, {x : 16, y : 1}, {x : 17, y : 1},
			    {x : 0, y : 2}, {x : 1, y : 2}, {x : 12, y : 2}, {x : 13, y : 2}, {x : 14, y : 2},
		...
		sphereStyle: function() {
			...
			for (i = 0; i < vm.length; i++) {
				...
				sphereStyle.push({
					transform: "rotateY("+(numG+1.3)*yDeg+"deg) rotateX("+(90-numC*xDeg)+"deg) translateZ(800px)"
				});	
			}
			return sphereStyle;
		},
		helixStyle: function() {
			...
			var h = 3.7,//环数
			    tY = 7,//每个li上下位移相差
			    num = Math.round(vm.length/h),//每环个数
			    deg = 360 / num,//每个li Y旋转相差
			    mid = vm.length/2 - 0.5;//找准中间点
			for (i = 0; i < vm.length; i++) {
				helixStyle.push({
					transform: "rotateY("+i*deg+"deg) translateY("+(i-mid)*tY+"px) translateZ(800px)"
				});	
			}
			return helixStyle;
		},
	...
	methods: {
		...
		changeShape(key) {
			let styleMap = {
				"table": vm.tableStyle,
				"sphere": vm.sphereStyle,
				"helix": vm.helixStyle,
				"grid": vm.gridStyle
			}
			let styleList = styleMap[key];
			for (var i = 0; i < vm.length; i++) {
			    vm.list[i].style = styleList[i];
			}
		},
	},
	created() {
